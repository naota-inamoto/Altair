...
void read_type_form(FILE *fp)
{
	...
	cell1 = s_read(fp);
	Type[i].name = (U_CHAR *)
		my_alloc(n = (sizeof(U_CHAR)*(int)strlen((const char*)_Atom(car(cell1)))) + 1);
	art_strcpy((char*)Type[i].name, n, (char*)_Atom(car(cell1)));
	cell1 = car(cdr(cell1));
	j = 1;
	while (!Null(cell2 = car(cell1))) {
		Form[i][j].name = (U_CHAR *)
		my_alloc(n = (sizeof(U_CHAR) *	
			  (int)strlen((const char*)_Atom(car(cell2)))) + 1);
		art_strcpy((char*)Form[i][j].name, n, (const char*)_Atom(car(cell2)));

		Form[i][j].gobi = (U_CHAR *)
		my_alloc(n = (sizeof(U_CHAR) *	
			  (int)strlen((const char*)_Atom(car(cdr(cell2))))) + 1);
		if (strcmp((const char*)_Atom(car(cdr(cell2))), "*") == 0)
		art_strcpy((char*)Form[i][j].gobi, n, "");
		else
		art_strcpy((char*)Form[i][j].gobi, n, (const char*)_Atom(car(cdr(cell2))));

		if (!Null(car(cdr(cdr(cell2))))) {
		/* 語尾の表記に漢字が混ざっている場合 */
		Form[i][j].gobi_yomi = (U_CHAR *)
			my_alloc(n = (sizeof(U_CHAR) *	
				  (int)strlen((const char*)_Atom(car(cdr(cdr(cell2)))))) + 1);
		if (strcmp((const char*)_Atom(car(cdr(cdr(cell2)))), "*") == 0)
			art_strcpy((char*)Form[i][j].gobi_yomi, n, "");
		else
		   art_strcpy((char*)Form[i][j].gobi_yomi, n, (const char*)_Atom(car(cdr(cdr(cell2)))));
		} else {
		Form[i][j].gobi_yomi = (U_CHAR *)
			my_alloc(n = sizeof(U_CHAR) *	
				 (int)strlen((const char*)Form[i][j].gobi) + 1);
		art_strcpy((char*)Form[i][j].gobi_yomi, n, (const char*)Form[i][j].gobi);
		}
